...
}.each do |description, variables|
    context description do
      shared_options = variables
...
EOM
              expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['--list-env']) }.to output(expected_output).to_stdout
            end
...
EOM
              expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-E']) }.to output(expected_output).to_stdout
            end

            it 'fails when environments cannot be determined from password dir option' do
              expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-E', '-d', @tmp_dir]) }.to raise_error(RuntimeError,
            ...
            it 'fails when environment directory does not exist' do
              ...
              expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-E']) }.to raise_error(RuntimeError,
...
EOM
              expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['--list-name']) }.to output(expected_output).to_stdout
            end
...
EOM
              expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-l']) }.to output(expected_output).to_stdout
            end
...
EOM
              expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-l', '-e', 'env1']) }.to output(expected_output).to_stdout
            end

            it 'fails when password directory does not exist' do
              FileUtils.rm_rf(@default_password_dir)
              expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-l']) }.to raise_error(RuntimeError,
...
EOM
              expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['--name', 'production_name2,production_name3']) }.to output(expected_output).to_stdout
            end
...
EOM
              expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-e', 'env1', '-n', 'env1_name1']) }.to output(expected_output).to_stdout
            end

            it 'fails when no names specified' do
              expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-n']) }.to raise_error(OptionParser::MissingArgument)
            end

            it 'fails when invalid name specified' do
              expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-n', 'oops']) }.to raise_error(
            ...
            it 'fails when password directory does not exist' do
              FileUtils.rm_rf(@default_password_dir)
              expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-n', 'production_name1']) }.to raise_error(RuntimeError,
                                                                                                     "Password directory '#{@default_password_dir}' does not exist")
            end
...
EOM
                expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['--backup', '-s', 'new_name']) }.to output(
              ...
              it 'allows multiple backups' do
                allow(::Utils).to receive(:get_password).and_return('new_password')
                Simp::Cli::Commands::Passgen.run(shared_options.dup + ['--backup', '-s', 'production_name1'])
                expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['--backup', '-s', 'production_name1']) }.not_to raise_error
                expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['--backup', '-s', 'production_name1']) }.not_to raise_error
              end
            end
...
EOM
                expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-e', 'env1', '--backup', '-s', 'new_name']) }.to output(
                  expected_output).to_stdout
            ...
            it 'fails when no names specified' do
              expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-s']) }.to raise_error(OptionParser::MissingArgument)
            end

            it 'fails when password directory does not exist' do
              FileUtils.rm_rf(@default_password_dir)
              expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-s', 'production_name1']) }.to raise_error(RuntimeError,
                                                                                                     "Password directory '#{@default_password_dir}' does not exist")
            end
...
EOM
                expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-r',
...
EOM
                expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-r', 'production_name1,production_name2',
...
EOM
                expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-e', 'env1', '-r',
...
EOM
                expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-e', 'env1', '-r', 'env1_name4',
                                                           '--force-remove']) }.to output(expected_output).to_stdout
            ...
            it 'fails when no names specified' do
              expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-r']) }.to raise_error(OptionParser::MissingArgument)
            end

            it 'fails when invalid names specified' do
              expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-r', 'production_name1,oops,production_name2']) }.to raise_error(
                OptionParser::ParseError,
            ...
            it 'fails when password directory does not exist' do
              FileUtils.rm_rf(@default_password_dir)
              expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-r', 'production_name1']) }.to raise_error(RuntimeError,
                                                                                                     "Password directory '#{@default_password_dir}' does not exist")
            end
          ...
          describe 'option validation' do
            it 'requires operation option to be specified' do
              ...
              expect { Simp::Cli::Commands::Passgen.run(shared_options.dup + ['-e', 'production']) }.to raise_error(OptionParser::ParseError,
